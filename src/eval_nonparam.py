...
def format_nonparam_content(instruction: str):
    return (
        "Task and available tools:\n"
        f"{instruction}\n\n"
        f"{NONPARAM_INSTRUCTION}"
    )


def format_nonparam_prompt(instruction: str, tokenizer, use_chat_template: bool):
    content = format_nonparam_content(instruction)
